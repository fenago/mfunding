-- 20260913a — the Processor board tells the truth about staleness and totals.
--
-- Owner report 9/13: "the processor tab, setter performance and the revenue
-- playbook are looking at the same merchants but where they are in the pipeline
-- is different, or they're not moving."
--
-- Two of the causes live in these two RPCs:
--
-- 1. `is_stale` was `days since created_at >= 14` while the bucket is labelled
--    "Two weeks, no traction". Measured 9/13: 128 of 155 open deals (83%) sat in
--    that bucket, and 22 of them had been worked in the previous 7 days. A
--    triage list holding 83% of the book is noise, and it was telling the
--    processor to nurture merchants they had just spoken to. Now computed from
--    the real last touch (newest logged activity or contact attempt), which the
--    function was ALREADY selecting as last_activity_at / last_contact_at and
--    simply not using. Also exposes `days_since_touch` so the UI can show it.
--
-- 2. `processor_stage_counts` counted EVERY deal with no filter, while
--    `processor_pipeline_rows` returns only `v_stages`. Same page, same board,
--    two totals (308 vs 288). The counts now take the same p_pipe and apply the
--    same stage set, so the header and the rows can never disagree again.
--    p_pipe defaults to 'mca' so the existing no-arg callers keep working.
...
begin
  ...
  enriched as (
    select b.*,
      ...
      ((select count(*)::int from public.processor_touches t where t.deal_id = b.id)
       + (select count(*)::int from public.activity_log al
           where al.entity_type = 'deal' and al.entity_id = b.id
             and al.interaction_type = 'call')) as touches_total,
      (exists(select 1 from public.processor_touches t
       ...
       or exists(select 1 from public.activity_log al
              where al.entity_type = 'deal' and al.entity_id = b.id
                and al.interaction_type = 'call'
                and (al.created_at at time zone 'America/New_York')::date
...
create or replace function public.processor_stage_counts(p_pipe text default 'mca')
returns jsonb
language plpgsql
stable security definer
set search_path to 'public'
...
begin
  ...
  -- MUST stay in lockstep with v_stages in processor_pipeline_rows — these two
  -- describe the same board and any difference shows up as two totals on one screen.
  -- Stage sets copied VERBATIM from processor_pipeline_rows — these two RPCs
  -- describe the same board, so any difference between them shows up as two
  -- different totals on one screen. Keep them in lockstep.
  if lower(coalesce(p_pipe, 'mca')) = 'vcf' then
    v_stages := array['new_distressed','hardship_consult','positions_analysis',
                      'strategy_proposal','agreement_sent','submitted_to_vcf',
                      'restructure_executed','servicing'];
