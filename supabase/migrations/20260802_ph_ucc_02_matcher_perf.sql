-- PH UCC matcher performance: precompute the normalized secured-party / alias
-- text as STORED generated columns and index the secured-party one with a trigram
-- GIN index, so ph_ucc_rebuild_leads() does a cheap indexed substring match
-- instead of running ph_ucc_norm() on every (filing × alias) pair at query time.
--
-- WHY: the first live run timed out — normalizing both sides inside the join is
-- ~O(filings × aliases) regexp work. As STORED columns the normalization happens
-- once at write time; the match becomes `sp_norm LIKE '%'||alias_norm||'%'`, which
-- pg_trgm accelerates. alias_norm is normalized to [A-Z0-9 ] only, so it can never
-- contain a LIKE metacharacter (% or _) — the concat is safe.

create extension if not exists pg_trgm;

alter table public.ph_ucc_filings
  add column if not exists sp_norm text
  generated always as (public.ph_ucc_norm(secured_party_raw)) stored;

alter table public.ph_ucc_funder_aliases
  add column if not exists alias_norm text
  generated always as (public.ph_ucc_norm(alias)) stored;

create index if not exists ph_ucc_filings_sp_norm_trgm
  on public.ph_ucc_filings using gin (sp_norm gin_trgm_ops);

-- Rebuilt matcher — identical semantics, indexed match. See original definition in
-- 20260802_ph_ucc_machine.sql for the score-formula commentary.
