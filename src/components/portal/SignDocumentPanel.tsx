...
interface SignDocumentPanelProps {
  documentId: string;
  /** Fired after a successful signature with the refreshed document, so a host
   *  (page or modal) can refresh surrounding state / close. */
  onSigned?: (doc: MerchantDocument) => void;
}

/**
 * The document viewer + signing flow, host-agnostic: used by the standalone
 * /portal/sign/:id page (email deep links) AND by the dashboard's in-page sign
 * modal. Renders the frozen merged_content as a properly formatted contract via
 * <MarkdownDoc> (the same safe, no-raw-HTML renderer the admin docs library
 * uses) — never as raw markdown.
 */
export default function SignDocumentPanel({ documentId, onSigned }: SignDocumentPanelProps) {
  ...
  return (
    <div className="space-y-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            {canSign
              ...
              : "Your signed copy is shown below."}
          </p>
        </div>
      </div>
      ...
      {/* Frozen document body — rendered as a formatted contract, never raw markdown */}
      <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
        <div className="max-h-[55vh] overflow-y-auto px-5 py-4 sm:px-8 sm:py-6">
          <MarkdownDoc source={doc.merged_content} />
        </div>
      </div>
