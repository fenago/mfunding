...
export async function updateDealStatus(id: string, newStatus: DealStatus): Promise<Deal> {
  ...
  // NOTE: GHL is the source of truth for the pipeline stage — we do NOT auto-push
  // stage changes to GHL here (that would fight the closers dragging deals in GHL).
  // Stage changes flow GHL -> app via the ghl-webhook. Deliberate, app-authoritative
  // writes to GHL happen explicitly (the "Sync to GHL" button, underwriting decline).
  // Commission creation on "funded" is handled by the source-agnostic DB trigger
  // `deals_commission_on_funded`, so it fires whether the deal is funded here or in GHL.
