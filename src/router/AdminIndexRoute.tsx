...
// The /admin index. Managers land on the dashboard; PURE setters are sent to the
// Setter Operations console — the simplified, application-first work surface
// (the "operations" tab on Setter Performance) — instead of the full Revenue
// Playbook, which they found confusing. Closers/managers keep the Playbook.
//
...
//
// Rollback: change the redirect target below back to "/admin/playbooks".
const AdminIndexRoute = () => {
  ...
  if (isCloserLens && !isManager)
    return <Navigate to="/admin/setter-performance?tab=operations" replace />;
