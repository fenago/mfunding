          ...
          {NUMBER_FIELDS.map((f) => (
            <div key={String(f.key)}>
              ...
              <div className="relative">
                ...
                <input
                  type="number"
                  value={(settings[f.key] as number | null) ?? ""}
                  onChange={(e) => setNumber(f.key, e.target.value)}
                  className={`input-field w-full ${f.suffix === "$" ? "pl-7" : ""} ${f.suffix === "%" ? "pr-8" : ""}`}
                />
                {f.suffix === "%" && (
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">%</span>
                )}
              </div>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{f.help}</p>
            </div>
          ))}
        </div>
      </div>
