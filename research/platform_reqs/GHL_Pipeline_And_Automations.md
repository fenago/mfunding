...
- [x] **TEST**: confirmed a real form submission creates a *deal* — verified June 29, 2026 on the LIVE site (see "Public `/apply` form wired to GHL" below).
...
- [x] **Public `/apply` form wired to GHL + pipeline-routing bug fixed** (June 29, 2026 — verified LIVE in production). Root cause: the live `/apply` page (`src/pages/ApplyPage.tsx`) was doing a raw insert into the standalone `funding_applications` table — nothing consumed it, so public applicants never reached GHL, the CRM, or any follow-up. (The homepage inline form `ApplySection.tsx` had been removed from the page in commit `b62f7f3` and left orphaned in the tree — now deleted.) **Fix:** `/apply` now calls the **`mca-intake`** edge function (same path as the calculators/assessments) → customer + MCA deal + GHL contact + MCA-pipeline opportunity at New Lead → fires Speed-to-Lead.
  - **Pipeline bug (affected ALL web intake, not just /apply):** `mca-intake`'s pipeline-selection heuristic matched `"new lead" && ("funded" || "renewal eligible")` and returned the *first* match — the **INACTIVE "MCA-Restructure-Pipeline"** (`dCJAhnyuZhNEUMgArH1c`) — so every lead from the Apply form, calculators, and assessments was landing in a dead pipeline (invisible to closers; the `ghl-webhook` stage-sync keys off the real pipeline, so stage changes never round-tripped). **Fixed** to target the canonical **MFunding MCA Pipeline by id** (`bG9ZEh4eP9x60E1CyaMx`), with a fallback heuristic that uniquely identifies it via the `renewal eligible` stage. Deployed as **`mca-intake` v3** (`verify_jwt=false` preserved).
  - **Verified end-to-end on the live site:** submitted `mfunding.net/apply` → network shows POST to `…/functions/v1/mca-intake` → deal created (`mca`/`new`, `lead_source=website_apply`) + GHL contact + opportunity **confirmed in pipeline `bG9ZEh4eP9x60E1CyaMx`**. All test data cleaned up (baseline 12 deals / 12 customers restored). Netlify auto-deployed from `main` (commits `279da5f` + `1dffca3`).
  - NOTE: `funding_applications` table KEPT (per user) — holds 1 apparently-real stranded lead (Cellit Services / Chris Stephenson / $125K / Apr 8 2026) that came in via the old broken path, plus 2 placeholder test rows. Decide later whether to recover the lead into the CRM or just hand off the contact details before dropping the table.
